...
\stopitemize
  ...
  \item In this training session, we recommend using the simplest of
    them: \type{picocom}
    \startitemize
    \item Installation with \type{sudo apt-get install picocom}
    \item Run with \type{picocom -b BAUD_RATE /dev/SERIAL_DEVICE}
    \item Exit with \type{Control-A Control-X}
    \stopitemize
  \item \type{SERIAL_DEVICE} is typically
    \startitemize
